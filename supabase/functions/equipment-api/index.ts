...
const API_VERSION = '2026-09-11-inspection-window-auto-1-10'
...
function defaultInspectionWindow(targetYear: number, targetMonth: number) {
  const y = String(targetYear).padStart(4, '0')
  const m = pad2(targetMonth)
  return {
    periodKey: `${y}-${m}-01 00:00:00`,
    targetYear,
    targetMonth,
    openDate: `${y}-${m}-01T00:00:00${BANGKOK_OFFSET}`,
    closeDate: `${y}-${m}-10T23:59:59${BANGKOK_OFFSET}`,
    isOverride: false,
    source: 'automatic',
  }
}
...
function fiscalMonths(fiscalYear: number) {
  const startYear = fiscalYear - 543
  return Array.from({ length: 12 }, (_, i) => ({
    y: i < 3 ? startYear - 1 : startYear,
    m: (9 + i) % 12 + 1,
  }))
}

function inspectionStatus(openDate: string, closeDate: string, now = new Date()) {
...
async function getInspectionWindows(months: { y: number, m: number }[]) {
  const { data, error } = await db.from('equipment_inspection_windows').select('*')
  if (error) throw error
  const overrides = new Map<string, any>()
  for (const row of data ?? []) overrides.set(`${Number(row.target_year)}-${pad2(Number(row.target_month))}`, row)
  const now = new Date()
  return months.map(({ y, m }) => {
    const fallback = defaultInspectionWindow(y, m)
    const override = overrides.get(`${y}-${pad2(m)}`)
    const hasOverride = !!override?.open_date && !!override?.close_date
    const openDate = hasOverride ? new Date(override.open_date).toISOString() : new Date(fallback.openDate).toISOString()
    const closeDate = hasOverride ? new Date(override.close_date).toISOString() : new Date(fallback.closeDate).toISOString()
    return {
      ...
      source: hasOverride ? 'admin' : 'automatic',
      status: inspectionStatus(openDate, closeDate, now),
      displayOpen: openDate,
      displayClose: closeDate,
    }
  })
}
...
async function inspectionWindowStatus(fiscalYearParam?: unknown) {
  const fiscalYear = Number(fiscalYearParam) || new Date().getFullYear() + 543
  const windows = await getInspectionWindows(fiscalMonths(fiscalYear))
  const current = windows.find(w => w.status === 'open') ?? null
  return { fiscalYear, isOpen: !!current, currentWindow: current, windows }
}
...
async function departmentReport(fiscalYear: unknown) {
  ...
  const windows = await getInspectionWindows(months)
  const windowByKey = new Map(windows.map(w => [w.periodKey, w]))
  const now = new Date()
  ...
  const rows = departments.map(department => {
    ...
    const monthRows = months.map(({ y, m }) => {
      const key = `${String(y).padStart(4,'0')}-${pad2(m)}-01 00:00:00`
      const window = windowByKey.get(key)!
      ...
      const isDue = now >= new Date(window.openDate)
      const isCompleted = isDue && inspected >= total
      const due = isDue ? total : 0
      return {
        display: `${inspected}/${total}`,
        inspected,
        total,
        due,
    ...
    const completedMonths = monthRows.filter(x => x.isCompleted).length
    const requiredMonths = monthRows.filter(x => x.isDue).length
...
async function handleGet(req: Request) {
  ...
  if (action === 'getApiInfo') return ok({ status: 'success', data: { version: API_VERSION, backend: 'Supabase' } })
  ...
  if (action === 'getInspectionWindowStatus') return ok({ status: 'success', data: await inspectionWindowStatus(url.searchParams.get('fiscalYear')) })
...
async function handlePost(req: Request) {
  ...
  if (action === 'saveInspectionWindow') {
    ...
    if (data.resetToDefault === true) {
      const periodKey = `${String(y).padStart(4,'0')}-${pad2(m)}-01 00:00:00`
      const { error } = await db.from('equipment_inspection_windows').delete().eq('period_key', periodKey)
      if (error) throw error
      return ok({ status: 'success', data: defaultInspectionWindow(y, m), message: 'คืนค่าเป็นรอบตรวจอัตโนมัติแล้ว' })
    }
    const openDate = isoOrNull(data.openDate)
    const closeDate = isoOrNull(data.closeDate)
    if (!openDate || !closeDate) return fail('กรุณาระบุวันเวลาเปิดและปิดรอบตรวจให้ครบ')
    if (new Date(openDate) >= new Date(closeDate)) return fail('วันเวลาเปิดรอบต้องก่อนวันเวลาปิดรอบ')
    const periodKey = `${String(y).padStart(4,'0')}-${pad2(m)}-01 00:00:00`
    const row = { period_key: periodKey, target_year: y, target_month: m, open_date: openDate, close_date: closeDate, updated_at: new Date().toISOString() }
  ...
  if (action === 'resetInspectionWindow') {
    const y = Number(data.targetYear), m = Number(data.targetMonth)
    if (!Number.isInteger(y) || !Number.isInteger(m) || m < 1 || m > 12) return fail('ปีหรือเดือนไม่ถูกต้อง')
    const periodKey = `${String(y).padStart(4,'0')}-${pad2(m)}-01 00:00:00`
    const { error } = await db.from('equipment_inspection_windows').delete().eq('period_key', periodKey)
  ...
  if (action === 'editEquipment') {
    ...
    if (row.rmc_no !== old.rmc_no) {
      ...
      const { data: saved, error } = await db.from('equipment_items').insert({ ...row, status: old.status, last_inspection_date: old.last_inspection_date, image_url: old.image_url, detail_link: old.detail_link, seq: old.seq }).select().single()
      if (error) throw error
      return ok({ status: 'success', data: { updatedItem: toApiItem(saved) } })
    }
    const { data: saved, error } = await db.from('equipment_items').update(row).eq('rmc_no', old.rmc_no).select().single()
    if (error) throw error
    return ok({ status: 'success', data: { updatedItem: toApiItem(saved) } })
  }

  if (action === 'deleteEquipment') {
    const item = await findItem(data.itemId, data.rmcNo)
    if (!item) return fail('ไม่พบครุภัณฑ์ที่ต้องการลบ', 404)
    const { error } = await db.from('equipment_items').delete().eq('rmc_no', item.rmc_no)
    if (error) throw error
    return ok({ status: 'success' })
  }

  if (action === 'pullEquipment') {
    const item = await findItem(data.itemId, data.rmcNo || data.originalItemId)
    if (!item) return fail('ไม่พบครุภัณฑ์ที่ต้องการย้าย', 404)
    const newWard = clean(data.newWard)
    if (!newWard) return fail('ไม่พบหน่วยงานปลายทาง')
    const now = new Date().toISOString()
    const { data: updated, error: ue } = await db.from('equipment_items').update({ usage_ward: newWard, status: 'ปกติ', last_inspection_date: now }).eq('rmc_no', item.rmc_no).select().single()
    if (ue) throw ue
    const { error: he } = await db.from('equipment_history').insert({ ts: now, item_id: item.item_id || item.rmc_no, inspector_name: clean(data.inspectorName) || 'ระบบ', ward: newWard, status: 'ย้ายหน่วยงาน', note: clean(data.note) || `ย้ายหน่วยงานไป ${newWard}` })
    if (he) throw he
  ...
  if (action === 'updateStatus' || action === 'editInspectionRecord') {
    const item = await findItem(data.itemId, data.rmcNo || data.originalItemId)
    if (!item) return fail('ไม่พบครุภัณฑ์ที่ต้องการบันทึก', 404)
    ...
    if (action === 'editInspectionRecord' && data.historyRowNumber) {
      ...
      if (Number.isFinite(historyId)) {
        const { data: h, error } = await db.from('equipment_history').update({ ts: now, inspector_name: clean(data.inspectorName), ward: clean(data.ward) || item.usage_ward || item.owner_ward, status: newStatus, note: clean(data.note), image_url: clean(data.existingImageUrl) || null }).eq('id', historyId).select().maybeSingle()
        if (error) throw error
        if (!h) return fail('ไม่พบบันทึกการตรวจที่ต้องการแก้ไข', 404)
      }
    } else {
      const { error } = await db.from('equipment_history').insert({ ts: now, item_id: item.item_id || item.rmc_no, inspector_name: clean(data.inspectorName) || clean(data.inspector) || '', ward: clean(data.ward) || clean(data.borrowedWard) || item.usage_ward || item.owner_ward || '', status: newStatus, note: clean(data.note), image_url: clean(data.existingImageUrl) || null })
      if (error) throw error
    }
    const { data: saved, error: se } = await db.from('equipment_items').update(update).eq('rmc_no', item.rmc_no).select().single()
    if (se) throw se
    return ok({ status: 'success', data: { updatedItem: toApiItem(saved) } })
  }
